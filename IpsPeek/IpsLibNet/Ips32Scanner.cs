...
namespace IpsLibNet
{
    public class Ips32Scanner
    {
        private bool _isIps32Patch;
        public string Ips32Header = "IPS32";
        private IpsScanner _ipsScanner;

        public Ips32Scanner(IpsScanner scanner)
        {
            _ipsScanner = scanner;
        }
        ...
        public List<IpsElement> Scan(Stream patch)
        {
            ...
            var patchHeaderString = System.Text.Encoding.ASCII.GetString(patchHeaderBytes);
            // Check header for 'PATCH.'
            // If patch is not valid...
            if (patchHeaderString != Ips32Header) return _ipsScanner.Scan(patch);

            _isIps32Patch = (Ips32Header == patchHeaderString);
            ...
            {
                // Add 4 bytes from patch stream to data (potentially containing 'EEOF').
                var data = Read(patch, 4, 1, 4);

                // If not the end of the file (no 'EOF' string found).
                if (!(System.Text.ASCIIEncoding.ASCII.GetString(data, 1, 3) == "EEOF"))
                {
                ...
                {
                    endOfFile = true;
                    patches.Add(new IpsEndOfFileValueElement((int)(patch.Position - 4)));
                }
            }
